...
public class AdminVoteControllerTest extends AbstractControllerTest {
    ...
    void getWithVotes() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL + RESTAURANT5_ID + "/with-user-votes"))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(RESTAURANT_WITH_VOTES_MATCHER.contentJson(restaurant5));
    }

    @Test
    @WithUserDetails(value = ADMIN_MAIL)
    void getAllWithVotes() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL + "/with-user-votes"))
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(RESTAURANT_GET_VOTES_MATCHER.contentJson(restaurantsWithUserVotes));
    }
